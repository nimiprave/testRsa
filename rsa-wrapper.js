...
rsaWrapper.generateClientServerKeys = ( ) => {
    let key = new NodeRSA();
    key.generateKeyPair(2048, 65537);
    fs.writeFileSync(path.resolve(__dirname, './keys', 'server.private.pem'), key.exportKey('pkcs8-private-pem'));
    fs.writeFileSync(path.resolve(__dirname, './keys',  'server.public.pem'), key.exportKey('pkcs8-public-pem'));
    fs.writeFileSync(path.resolve(__dirname, './keys',  'client.public.pem'), key.exportKey('pkcs8-public-pem'));
